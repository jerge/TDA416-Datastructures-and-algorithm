...
public class Main {
	...
	public static void drawShape(Iterator it, DrawGraph shape, Color c, boolean layer) {
		...
		while (it.hasNext()) {
			...
			shape.drawLine((int) p1.getX(), (int) p1.getY(), (int) p2.getX(), (int) p2.getY(), c, 2.0, l);
			p1 = p2;
	...
	public static void main(String[] args) {
		...
		while (j + 1 < args.length && args[j + 1].charAt(0) == '-') {
			...
			case 'h': {
				...
				break;
			/*
			 * case 'w' : { try { w = Integer.parseInt( args[j].substring(2) ); } catch (
			 * NumberFormatException e ) {
			 * System.out.println("main: w must have numerical argument"); System.exit(0); }
			 * } break; case 'h' : { try { h = Integer.parseInt( args[j].substring(2) ); }
			 * catch ( NumberFormatException e ) {
			 * System.out.println("main: h must have numerical argument"); System.exit(0); }
			 * } break;
			 */
			default:
		...
		// read from user/file
		if (debug)
			System.out.println("*** reading points from user");
		outline.readShape();
		// create graphics area
		int x = outline.getMaxX();
		int y = outline.getMaxY();
		if (debug)
			System.out.println("*** maximum x coordinate is: " + x + " maximum y coordinate is: " + y);
		...
		if (debug)
			System.out.println("************ draws original shape in black in graphics area");
		drawShape(it, shape, Color.BLACK, true);
		outline.reduceListToKElements(k);
		if (debug)
			System.out.println("************ this is the list after remove to k points:");
		if (debug)
			System.out.println(outline);
		/*
		 * try{ // take a breake - oförklarliga timing problems med uppritningen
		 * outline.wait(1000); } catch (Exception e ) { }
		 */
		if (debug)
			System.out.println("********** draws generated shape with k points in red");
		it = outline.iterator();
